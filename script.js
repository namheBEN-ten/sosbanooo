// =========================================================
  ...
  function updateIdentityUI(){
    ...
    if(customer){
      ...
      document.getElementById('identity-badge-name').textContent = customer.username;
      document.getElementById('identity-badge-phone').textContent = customer.phone;
  ...
  async function renderAdminChat(){
    ...
    list.innerHTML = '<p class="empty-note">در حال بارگذاری مکالمات...</p>';
    try {
      ...
      list.innerHTML = '<p class="empty-note">دریافت مکالمات ناموفق بود.</p>';
    }
  }
  ...
  function renderThread(sessionIdValue){
    ...
    thread.innerHTML = `
      ...
      <div class="thread-reply-row">
        <input id="thread-reply-input" placeholder="پاسخ به مشتری..." onkeydown="if(event.key==='Enter'){replyToChat('${escapeHtml(sessionIdValue)}')}">
